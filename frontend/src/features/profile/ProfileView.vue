...
<template>
  <PageState :loading="loading" :error="error" @retry="load">
    ...
    <section class="panel sessions-panel">
      ...
      <div class="session-list"><article v-for="session in sessions" :key="session.id" class="session-row"><div class="session-device"><span class="session-icon"><Icon name="monitor" :size="18" /></span><div><strong>{{ session.device || '未知设备' }}</strong><small><span class="session-online" />{{ session.ip || 'IP 未记录' }}</small></div></div><div class="session-meta"><span><small>最后活动</small><time>{{ date(session.last_login_at || session.created_at, true) }}</time></span><button class="session-remove" type="button" @click="removeSession(session.id)"><Icon name="x" :size="14" />移除</button></div></article></div>
