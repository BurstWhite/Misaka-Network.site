...
const planFeatures = computed(() => [
  ['总流量', bytes(plan.value.transfer_enable || total.value)],
...
function openClient(client: string) {
  ...
  const links: Record<string, string> = {
    clash: `clash://install-config?url=${encoded}&name=${encodeURIComponent('Misaka Subscription')}`,
    shadowrocket: `shadowrocket://add/sub?url=${encoded}`,
    v2rayn: `v2rayn://install-config?url=${encoded}`,
  }
  window.location.assign(links[client])
}
...
<template>
  <PageState :loading="loading" :error="error" @retry="load">
    ...
    <section class="detail-grid subscription-grid">
      <article class="panel plan-details"><header><div><h2>套餐内容</h2><p>当前套餐的可用权益</p></div><span class="plan-badge">进行中</span></header><div class="feature-list"><div v-for="([label, value]) in planFeatures" :key="label"><span>{{ label }}</span><strong>{{ value }}</strong></div></div><p v-if="plan.content" class="plan-content">{{ plan.content }}</p></article>
