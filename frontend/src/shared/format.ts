...
export function shortDate(value: number | string | undefined): string { if (!value) return '—'; const stamp = typeof value === 'number' && value < 1e12 ? value * 1000 : value; const parsed = new Date(stamp); return Number.isNaN(parsed.getTime()) ? String(value) : `${parsed.getMonth() + 1}.${parsed.getDate()}` }
