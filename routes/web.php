...
<style data-xboard-auth-layout-patch="1">
  ...
  body.xboard-auth-page .xboard-auth-logo-source{display:none!important}
...
(function(){
  ...
  function logoCandidate(card){var imgs=[].slice.call(card.querySelectorAll('img')).filter(function(img){return visible(img)&&!(img.closest&&img.closest('.xboard-auth-brand'))});imgs.sort(function(a,b){var ar=a.getBoundingClientRect(),br=b.getBoundingClientRect();return br.width*br.height-ar.width*ar.height});return imgs[0]||null}
  function normalizedImageUrl(value){if(!value)return'';var anchor=document.createElement('a');anchor.href=value;return anchor.href}
  function ensureLogo(brand,card){var logo=brand.querySelector('[data-xboard-auth-logo="1"]'),source=logoCandidate(card);if(!logo){var sourceUrl=settings.logo||(source&&(source.currentSrc||source.src));if(!sourceUrl)return;logo=document.createElement('img');logo.src=sourceUrl;logo.alt=settings.title||'XBoard';logo.setAttribute('data-xboard-auth-logo','1');brand.insertBefore(logo,brand.firstChild||null)}logo.classList.add('xboard-auth-logo');logo.loading='eager';logo.decoding='async';var logoUrl=normalizedImageUrl(logo.currentSrc||logo.src);[].slice.call(card.querySelectorAll('img')).forEach(function(img){if(img===logo||(img.closest&&img.closest('.xboard-auth-brand')))return;if(logoUrl&&normalizedImageUrl(img.currentSrc||img.src)===logoUrl)img.classList.add('xboard-auth-logo-source')})}
