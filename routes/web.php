...
(function(){
  ...
  function markLinks(card){var list=[].slice.call(card.querySelectorAll('div,section,footer')).filter(function(n){if(n.classList.contains('xboard-auth-brand'))return false;var text=String(n.textContent||'').replace(/\\s+/g,' ');return /注册|忘记密码|简体中文|English|语言|language/i.test(text)&&n.querySelectorAll('a,button,[role=button]').length});if(!list.length)return;list.sort(function(a,b){return b.getBoundingClientRect().top-a.getBoundingClientRect().top});list[0].classList.add('xboard-auth-links')}
